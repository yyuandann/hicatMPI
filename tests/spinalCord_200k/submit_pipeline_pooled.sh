...
#
# TEST RUN: spinal cord neurons (194,221 cells x 17,277 genes, 32-d scVI latent) -- the dataset
# every R-alignment report (docs/tests_aligning_to_bigcat, reports 1-9) was validated on. The
# parameter blocks below are untouched example defaults = the validated alignment configuration
# (score_thresh [300,150], seed 2024 via the manager default, pynndescent knn_seed=1).
# Reference outcome: ~454 clusters pre final merge, ~428 after (report 4/5 era numbers; the
# pynndescent default and later fixes shift this slightly -- see report 9).
#
...
adata_path="/allen/programs/celltypes/workgroups/mct-t200/Manuscript/2025_SpinalCord/Initial_submission/Common/RNA/pre_merging/integrated_clustering_scvi_large_HVG_4000_combined_batch_recode_rm_DRG_clean_v2_neuron/spcdata_subset_neuron_subset.h5ad"
latent_path="/allen/programs/celltypes/workgroups/mct-t200/Manuscript/2025_SpinalCord/Initial_submission/Common/RNA/pre_merging/integrated_clustering_scvi_large_HVG_4000_combined_batch_recode_rm_DRG_clean_v2_neuron/X_scVI_rename_subset.csv"
out_dir="/allen/programs/celltypes/workgroups/rnaseqanalysis/dyuan/hicatMPI/tests/spinalCord_200k"
